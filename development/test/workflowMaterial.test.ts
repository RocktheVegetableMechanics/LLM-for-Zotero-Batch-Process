...
describe("generated workflow material", function () {
  it("binds an unscoped full read to its captured paper without accepting another paper's evidence", function () {
    const r = request();
    const paper = {
      itemId: 42,
      contextItemId: 43,
    ...
    r.classifiedIntent!.semantic!.reading.coverage = "exhaustive";
    r.turnPaperScope = {
      ...r.turnPaperScope,
      papers: [{ paper, roles: ["active"] }],
    };
    const obligation = r.actionContract!.obligations[0];
    obligation.operation = "read_full";
    delete obligation.targetBoundary;
    r.actionProgress!.obligations[0].status = "fulfilled";
    const readGateway = { ...gateway, listPaperContexts: () => [paper] };
    ...
    assert.throws(
      () => assertMaterialReady(r, output, readGateway),
      /Read the requested source/,
    );
    r.documentReadObservations[0].itemKey = "KEY42";
    assert.doesNotThrow(() => assertMaterialReady(r, output, readGateway));
    // An explicit but unresolved boundary must not silently expand to the active paper.
