...
import {
  ...
  resolveCommandPath,
} from "../src/agent/tools/write/commandAnalysis";
...
describe("shell effect analysis for automatic execution", function () {
  it("retains native Windows output paths for IOUtils and recovery", function () {
    assert.equal(
      resolveCommandPath("report.txt", "C:\\Reports\\run"),
      "C:\\Reports\\run\\report.txt",
    );
    assert.equal(
      resolveCommandPath("C:/Reports/run/../report.txt", undefined),
      "C:\\Reports\\report.txt",
    );
    assert.equal(
      resolveCommandPath("../report.txt", "/tmp/run"),
      "/tmp/report.txt",
    );
  });
  ...
  beforeEach(function () {
    (globalThis as any).Zotero = { isWin: false, isMac: true }; // POSIX fixtures; Windows cases override this explicitly.
  ...
  it("recognizes only argument-free Windows cd as a directory read", async function () {
    (globalThis as any).Zotero = { isWin: true };
    assert.equal((await decision("cd")).kind, "execute");
    assert.equal((await decision("cd C:\\Users")).kind, "model_review");
  });
  for (const command of [
