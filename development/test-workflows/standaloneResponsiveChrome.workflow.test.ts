          ...
          for (const width of [320, 340, 380, 500]) {
            ...
            for (const [index, rect] of buttons.entries()) {
              ...
              for (const other of buttons.slice(index + 1)) {
                ...
                assert.isFalse(
                  overlaps,
                  `Header buttons overlap: ${context}; ${JSON.stringify({ rect: rect.toJSON(), other: other.toJSON(), compression: header.querySelector<HTMLElement>(".llm-header-top")!.style.getPropertyValue("--llm-runtime-compression") })}`,
                );
              }
            }
          }
