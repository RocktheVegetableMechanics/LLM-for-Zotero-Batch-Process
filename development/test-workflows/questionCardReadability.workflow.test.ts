...
describe("workflow: question card readability", function () {
  ...
  it("contains long choice text and keeps the active question visible after resizing", async function () {
    ...
    try {
      ...
      // This helper renders a pending card without a conversation/run record.
      // An unrelated asynchronous chat refresh may therefore remove it from
      // .llm-messages. Measure the real, interactive card in an isolated host.
      const root = doc.createElement("div");
      root.className = "llm-panel";
      root.style.cssText =
        "position:fixed;left:0;top:0;width:600px;height:650px";
      ...
      try {
        for (const [width, scale] of [
          ...
          assert.isTrue(
            card.isConnected,
            "The question card must remain attached while pending",
          );
          assert.isTrue(
            root.isConnected,
            "The paper panel must remain attached while pending",
          );
