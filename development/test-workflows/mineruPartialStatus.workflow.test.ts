...
async function preferences(id: number, status: string): Promise<Window> {
  ...
  if (status === "partial") {
    // Coverage is populated asynchronously after the row and status dot mount.
    await waitFor(() =>
      Boolean(
        (win.document.querySelector(
          `[data-parent-id="${id}"] [data-status="partial"]`,
        ) as HTMLElement | null)?.title.includes("200/401"),
      ),
    );
  }
  return win;
