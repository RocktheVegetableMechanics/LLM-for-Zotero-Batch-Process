import { openSequentialPaperBatch } from "./paperBatch";
...
export function setupHandlers(
  ...
  const batchButton = body.ownerDocument.createElementNS(
    "http://www.w3.org/1999/xhtml",
    "button",
  );
  batchButton.textContent = "加入批量队列";
  batchButton.setAttribute("type", "button");
  batchButton.addEventListener("click", () =>
    openSequentialPaperBatch(
      { ...getSelectedProfile(), reasoning: getSelectedReasoning() },
      resolveDisplayConversationKind(item) === "paper" ? item : null,
      body.ownerDocument,
    ),
  );
  body.querySelector("#llm-send")?.parentElement?.append(batchButton);
