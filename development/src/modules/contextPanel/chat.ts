import { createRetryablePersistence } from "./paperBatchRuntime";
...
export async function loadStoredConversationByKey(
  conversationKey: number,
  limit: number,
  conversationSystem?: ConversationSystem | null,
...
export function resolveEffectiveRequestConfig(params: {
  ...
  if (isCodexAppServerConversationRequest(params)) {
    ...
    const reasoningMode = params.reasoningMode ?? getCodexReasoningModePref();
    const reasoning =
      params.reasoningMode !== undefined
        ? buildCodexAppServerReasoningConfig(reasoningMode)
...
export async function sendQuestion(
  ...
  let codexActivityTrace: CodexNativeActivityTraceController | null = null;
  const persistAssistantOnce = createRetryablePersistence(
    async (status?: import("../../agent/types").AgentRunStatus) => {
  ...
  try {
    ...
    return {
      status:
        assistantMessage.completionStatus &&
        assistantMessage.completionStatus !== "complete"
          ? "blocked"
          : "completed",
      detail: assistantMessage.completionReason,
    } as import("./paperBatchRuntime").PaperBatchOutcome;
  } catch (err) {
    const isCancelled =
      getCancelledRequestId(conversationKey) >= thisRequestId ||
      Boolean(getAbortController(conversationKey)?.signal.aborted) ||
    ...
    return { status: "failed", detail: errMsg } as const;
  } finally {
