import { buildAgentRecoveryInstruction } from "./completion";
import {
...
export class CodexResponsesAgentAdapter implements AgentModelAdapter {
  ...
  async runStep(params: AgentStepParams): Promise<AgentModelStep> {
    ...
    let normalized: Awaited<ReturnType<typeof parseResponsesStepStream>>;
    try {
      ...
      if (
        ...
        throw error;
      // The parser did not return a completed call. Keep only inputs whose
      // execution already finished; the runtime's bounded recovery discards deltas.
