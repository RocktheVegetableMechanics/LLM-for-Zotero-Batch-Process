import { resolveFullReadTargets } from "../tools/read/paperRead";
  ...
  const targetIds = [
    ...new Set(
      output.sourceActionIndexes.flatMap((index) =>
        obligationsForAction(contract, index).flatMap((obligation) => {
          if (obligation.targetBoundary)
            return obligation.targetBoundary.frozenTargetIds;
          if (obligation.operation !== "read_full" || obligation.scope)
            return [];
          return resolveFullReadTargets({
            input: { mode: "full" },
            context: {
              request: {
                ...request,
                actionContract: { ...contract, obligations: [obligation] },
              },
            } as AgentToolContext,
