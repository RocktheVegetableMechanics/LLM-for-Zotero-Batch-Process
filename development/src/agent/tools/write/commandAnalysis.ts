...
export function resolveCommandPath(
  ...
  const normalize = (value: string): string => {
    ...
    // These paths reach IOUtils and the durable write journal, both of which
    // require native Windows paths rather than slash-normalized display paths.
    return joinLocalPath(
      `${prefix}${segments.join("/")}` || (absolute ? prefix : "."),
    );
  };
