...
export function isReadOnlyCommand(command: ShellCommand): boolean {
  ...
  // cmd.exe's argument-free cd prints the working directory. With arguments
  // it changes shell state and must go through the normal classification.
  if (
    program === "cd" &&
    !args.length &&
    getRuntimePlatformInfo().platform === "windows"
  )
    return true;
